	...
	const execRssReader = (feedsStr) => {
				...
				getXml(feedsArr[i].url, items => {
					...
					if (Array.isArray(items)) {
						...
						for (let j = 0; j < nitems.length; j++) {
							...
							// console.log(123, nitems[j].title?._text)
							...
							else nitems[j].enclosure = { ...nitems[j].enclosure._attributes }
		...
		const getXml = (url, cb) => {
			api.call("ressource.fetch", [url, { disableCache: true }], txt => {
				...
				// only take x first elements
				items = items.slice(0, opts.itemsPerFeed)
