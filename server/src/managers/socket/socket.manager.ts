...
export interface iSockerRoute { event: string, action: iServerSocketAction, disableDataLog?:boolean}
    ...
    ioServer.on(socketEvents.connection, (socket, params) => {
        ...
        socketRoutes.forEach(route => {
            socket.on(route.event, (data) => {
                sharedConfig.debug.socketEvents && console.log(`[SOCKET SERV EVENT] --> ${route.event} `);
                if (sharedConfig.debug.socketEvents && !route.disableDataLog) console.log(`with data `, data);
